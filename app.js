  ...
  async function loadSeriesList() {
    ...
    try {
      ...
      const cards = series.map((s) => {
        ...
        title.textContent = titleFromSlug(s.name);

        a.append(thumb, title);
        li.appendChild(a);
        list.appendChild(li);

        return { slug: s.name, thumb };
      });

      await Promise.all(
        cards.map(async ({ slug, thumb }) => {
          try {
            const photos = await fetchSeriesPhotos(slug);
            const pick = pickRandomPhoto(photos);
            if (pick) {
              attachSeriesThumb(thumb, slug, pick, imagesRoot);
            }
          } catch {
            /* skip thumb if series cannot be loaded */
          }
        })
      );
    } catch (err) {
